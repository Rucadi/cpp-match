...
Result<double, std::string> safe_str_to_double(const std::string & str)
{
    ...
    if (ptr != str.data() + str.size())
        return "Extra characters found in number";
...
Result<Coordinate, std::string> parse_coordinate(const std::string & input)
{
    ...
    double latitude = expect_e(safe_str_to_double(lat_str));
    double longitude = expect_e(safe_str_to_double(lon_str));
...
Result<std::vector<Coordinate>, std::string> parse_coordinates(const std::string & input)
{
    ...
    while (std::getline(ss, token, ';')) {
        ...
        Coordinate coord = expect_e(parse_coordinate(token));
...
void test_parser()
{
    ...
    for (const auto & test : test_cases) {
        ...
        match_e(
            ...
            [](const std::string & err) { std::print("{}Error: {}{}\n", RED, err, RESET); });
    }
}
